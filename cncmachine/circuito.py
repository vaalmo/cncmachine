...
def crear_modelo_datos(i):
    """Almacena los datos del problema de circuitos."""
    
    
        # ubicaciones de los puntossda del circuito
    data['lugares'] = [
    ...
    data['actuador'] = 1  #punta con soldadura o broca -- 1 es el numero de actuadores que tengo 
    data['deposito'] = i  #punto según el indice -- punto de partida y llegada 
...
def mostrar_solucion(controlador, enrutamiento, solucion):
    ...
    while not enrutamiento.IsEnd(index):
        plan_output += ' data[lugar:{}] ->'.format(controlador.IndexToNode(index))
...
def calcular_solucion(data):
    ...
    solucion2 = solucion.ObjectiveValue()
...
def main():
    ...
    i = 0
    costos = []
    length_costos = len(costos)<281

    while(i < 280):
        for j in range (length_costos):
        
         data = crear_modelo_datos(i)
         solucion = calcular_solucion(data)
    ...
    print("Los puntos de inicio, con los costos mas bajos (mas optimos) son: ")
    print('[249] ->', costos[0])
    print('[10] ->', costos[1])
    print('[11] ->', costos[2])
    print('[7] ->', costos[3])
    ...
    costos.sort(reverse = True)

    print("Los puntos de inicio, con los costos mas altos (menos optimos) son: ")
    print('[62] ->', costos[0])
    print('[75] ->', costos[1])
    print('[95] ->', costos[2])
    print('[96] ->', costos[3])
    ...
    print(f"El punto de inicio optimo es: {controlador_menor}, con costo de {menor}")
    print(f"El punto de inicio menos optimo es: {controlador_mayor}, con costo de {mayor}")
